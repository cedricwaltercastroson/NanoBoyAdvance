...
void PPU::OnHblankComplete(int cycles_late) {
  ...
  if (vcount == 160) {
    ...
    BeginRenderBG();
    scheduler.Add(46 - cycles_late, this, &PPU::BeginComposer);
...
void PPU::Sync() {
  // TODO: instead maintain "active" flags for background renderer, OBJ renderer and composer?
  if (mmio.dispstat.hblank_flag || mmio.dispstat.vblank_flag) {
    return;
  }

  auto timestamp_now = scheduler.GetTimestampNow();

  // background
  {
    auto cycles = int(timestamp_now - renderer.timestamp);

    // TODO: latch BG mode at the the scanline start?
    ...
    renderer.timestamp = timestamp_now;
  }

  // composer
  if (composer.engaged) {
    auto cycles = int(timestamp_now - composer.timestamp);

    Compose(cycles);

    composer.timestamp = timestamp_now;
  }
}
